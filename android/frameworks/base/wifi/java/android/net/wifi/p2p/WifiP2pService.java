...
public class WifiP2pService extends IWifiP2pManager.Stub {
    ...
    private static final String WFD_P2P_DEVICE_ADDR = "wlan.wfdp2p.addr";

    /* Is chosen as a unique range to avoid conflict with
       the range defined in Tethering.java */
    private static final String[] DHCP_RANGE = {"192.168.49.2", "192.168.49.254"};
    private static final String SERVER_ADDRESS = "192.168.49.1";
    private static final String SERVER_ADDRESS_TETHER = "192.168.43.1";//copy from Tethering.java
    //tether mode with p2p
    ...
    public WifiP2pService(Context context) {
        ...
        SystemProperties.set(WFD_P2P_DEVICE_ADDR, "");//empty
    ...
    private class P2pStateMachine extends StateMachine {
        ...
        private WifiP2pDeviceList mNotP2pDevice = new WifiP2pDeviceList();
            ...
            switch (message.what) {
                ...
                case WifiP2pManager.SET_WFD_INFO:
                    replyToMessage(message, WifiP2pManager.SET_WFD_INFO_FAILED,
                            WifiP2pManager.BUSY);
                    break;
                case WifiP2pManager.SET_GO_PSK:
                    replyToMessage(message, WifiP2pManager.SET_GO_PSK_FAILED,
                            WifiP2pManager.BUSY);
                    break;                    
    ...
    class P2pEnabledState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiP2pManager.SET_GO_PSK:
                    WifiP2pDevice d3 = (WifiP2pDevice) message.obj;
                    String goPsk = d3.deviceName;
                    if (goPsk != null && (goPsk.length() < 13) && (goPsk.length() > 7) && setGroupOwnerPsk(goPsk)) {
                        logd("set GroupOnwer psk to [" + goPsk + "]");
                        replyToMessage(message, WifiP2pManager.SET_GO_PSK_SUCCEEDED);
                    } else {
                        replyToMessage(message, WifiP2pManager.SET_GO_PSK_FAILED,
                                WifiP2pManager.ERROR);
                    }
...
//                setWifiP2pInfoOnGroupFormation(SERVER_ADDRESS);
                ...
                if(mAutonomousGroup)
                    serverAddr = SERVER_ADDRESS_TETHER;
                else
                    serverAddr = SERVER_ADDRESS;
                setWifiP2pInfoOnGroupFormation(serverAddr); 
            }
                ...
                case WifiMonitor.AP_STA_CONNECTED_EVENT:
                    ...
                    if (deviceAddress != null) {
                        if (mSavedProvDiscDevice != null &&
                            ...
                            notP2pDevice = false;
                            loge("GroupCreatedState: p2p device connect action............!");    
                        }                       
                        ...
                        if(notP2pDevice) {
	                        ...
	                        {
	                        	notP2pDevice = false;
	                        	loge("GroupCreatedState: p2p device connect action............001!");    
	                        }
	                        else
	                        {                     
                            	mNotP2pDevice.update(device);
                            	loge("GroupCreatedState: not p2p device connect action........!");                                                     
                            	break;
                            }
                        }
                        ...
                        if(mWifiP2pDevice == null && device.wfdInfo != null)
                        {
                        ...
                        sendP2pPeersChangedBroadcast();
                        sendP2pConnectionChangedBroadcast();//miracast give longer time to waiting for getPeeripAddress.
                    } else {
                        loge("Connect on null device address, ignore");
                    }
                    loge("GroupCreatedState: AP_STA_CONNECTED_EVENT end!!!!");                    
                    break;
                case WifiMonitor.AP_STA_DISCONNECTED_EVENT:
                    device = (WifiP2pDevice) message.obj;
                    deviceAddress = device.deviceAddress;
                    if (deviceAddress != null) {
                        if(null != mNotP2pDevice.get(deviceAddress)) {
                            loge("GroupCreatedState: AP_STA_DISCONNECTED_EVENT not p2p device break;!!!!");
                            mNotP2pDevice.remove(device);
                            break;
                        }
        ...
        try {
            ...
            if(mAutonomousGroup) {
                ...
                if(mCm.tether(intf) != ConnectivityManager.TETHER_ERROR_NO_ERROR) {
                    loge("p2p: Error tethering on " + intf);
                    return;
                }
                logd("p2p: mTetherInterfaceName :" + intf);
...
/*
    ...
    private boolean setGroupOwnerPsk(String goPsk) {
        if (goPsk == null) return false;

        if (!mWifiNative.setGroupOwnerPsk(goPsk)) {
            loge("Failed to set GroupOwner psk: [" + goPsk+ "]");
            return false;
        }
        return true;
    }
