...
public class WifiP2pDevice implements Parcelable {
     ...
     *  
     */
	private static final Pattern fourDevicePattern = Pattern.compile(
        "(?:[0-9a-f]{2}:){5}[0-9a-f]{2} p2p_dev_addr=((?:[0-9a-f]{2}:){5}[0-9a-f]{2}) wfd_dev_info=0x000006([0-9a-fA-F]{12})"
    );
    ...
    public WifiP2pDevice(String string) throws IllegalArgumentException {
        ...
        switch (tokens.length) {
            ...
            case 3:
                ...
                return;
            case 4:
                match = fourDevicePattern.matcher(string);                
                if (!match.find()) {
                    throw new IllegalArgumentException("Malformed supplicant event");
                }
                deviceAddress = match.group(1);
