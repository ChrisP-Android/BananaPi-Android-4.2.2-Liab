...
SampleTable::~SampleTable() {
    ...
    removeChunkOffsetParams();
    removeSampleSizeParams();
...
status_t SampleTable::saveChunkOffsetParams(
        uint32_t type, off64_t data_offset, size_t data_size) {

    CHECK(type == kChunkOffsetType32 || type == kChunkOffsetType64);  
    
    mBufstco = (char *)malloc(data_size);
    if (mBufstco == NULL)
    {
        ALOGE("Be careful! malloc failed");
        return ERROR_MALFORMED;
    }
    //memset(mBufstco, 0, data_size);
    if (mDataSource->readAt(
                (data_offset + 8), (mBufstco + 8), (data_size - 8)) < (ssize_t)(data_size - 8)) {
        return ERROR_IO;
    }
    mBufChunkOffsetOffset = (uint32_t)mBufstco;

    return OK;
}

status_t SampleTable::removeChunkOffsetParams() {
    if (mBufstco != NULL) {
        free (mBufstco);
        mBufstco = NULL;
    }
...
status_t SampleTable::saveSampleSizeParams(
        uint32_t type, off64_t data_offset, size_t data_size) {

    CHECK(type == kSampleSizeType32 || type == kSampleSizeTypeCompact);
    
    mBufstsz = (char *)malloc(data_size);
    if (mBufstsz == NULL)
    {
        ALOGE("Be careful! malloc failed");
        return ERROR_MALFORMED;
    }
    //memset(mBufChunkOffsets, 0, data_size);
    if (mDataSource->readAt(
                (data_offset + 12), (mBufstsz + 12), (data_size - 12)) < (ssize_t)(data_size - 12)) {
        return ERROR_IO;
    }

    mBufSampleSizeOffset = (uint32_t)mBufstsz;

    return OK;
}

status_t SampleTable::removeSampleSizeParams() {
    if (mBufstsz != NULL) {
        free (mBufstsz);
        mBufstsz = NULL;
    }
