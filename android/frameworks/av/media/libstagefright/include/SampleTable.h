...
public:
    ...
    status_t saveChunkOffsetParams(
            uint32_t type, off64_t data_offset, size_t data_size);
    
    status_t removeChunkOffsetParams();

    status_t saveSampleSizeParams(
                uint32_t type, off64_t data_offset, size_t data_size);

    status_t removeSampleSizeParams();
...
private:
    ...
    char *  mBufstco;
    uint32_t mBufChunkOffsetOffset;
