...
static create_aw_omx_component omx_core_load_cmp_library(int idx)
{
	...
	if(core[idx].so_lib_handle)
	{
		if(core[idx].fn_ptr == NULL)
		{
			...
			if(core[idx].fn_ptr == NULL)
			{
				ALOGV("Error: Library %s incompatible as QCOM OMX component loader - %s\n", core[idx].so_lib_name, dlerror());
			}
		}
	}
	else
	{
		ALOGV("Error: Couldn't load %s: %s\n",core[idx].so_lib_name,dlerror());
	}
...
static int get_cmp_index(char *cmp_name)
{
	...
	ALOGV("before get_cmp_index **********%d\n", rc);

	pthread_mutex_lock(&g_mutex_core_info);

	for(i=0; i< (int)SIZE_OF_CORE; i++)
	{
		ALOGV("get_cmp_index: cmp_name = %s , core[i].name = %s ,count = %d \n",cmp_name,core[i].name,i);
	...
	for(i=0; i< SIZE_OF_CORE; i++)
	{
		if(!strcmp(cmp_name, core[i].name))
		{
			for(j=0; j< OMX_COMP_MAX_INST; j++)
			{
				...
				{
					rc = j;
					ALOGV("free handle slot exists %d\n", rc);
...
static int check_lib_unload(int index)
{
	...
	if(rc == 1)
	{
		ALOGD(" Unloading the dynamic library for %s\n", core[index].name);
		err = dlclose(core[index].so_lib_handle);
		if(err)
		{
			ALOGV("Error %d in dlclose of lib %s\n", err,core[index].name);
		}
...
void* get_cmp_handle(char *cmp_name)
{
	...
	ALOGV("get_cmp_handle returning NULL \n");
...
OMX_API OMX_ERRORTYPE OMX_APIENTRY OMX_Deinit()
{
	...
	for(i=0; i< SIZE_OF_CORE; i++)
	{
		for(j=0; j< OMX_COMP_MAX_INST; j++)
		{
			if(core[i].inst[j])
			{
				ALOGV("OMX DeInit: Freeing handle for %s\n", core[i].name);
		...
		if(cmp_index >= 0)
		{
			...
			if(fn_ptr)
			{
				...
				if(pThis)
				{
					...
					ALOGV("Component %x Successfully created\n",(unsigned)*handle);
				}
				else
				{
			...
			else
			{
				...
				ALOGV("library couldnt return create instance fn\n");
			}
		}
		else
		{
			eRet = OMX_ErrorNotImplemented;
			ALOGV("ERROR: Already another instance active  ;rejecting \n");
		}
	}
	else
	{
...
OMX_API OMX_ERRORTYPE OMX_APIENTRY OMX_FreeHandle(OMX_IN OMX_HANDLETYPE hComp)
{
	...
	ALOGV("OMXCORE API :  Free Handle %x\n",(unsigned) hComp);
	...
	if((i= is_cmp_handle_exists(hComp)) >=0)
	{
		...
		{
			ALOGV(" OMX_FreeHandle failed on %x\n",(unsigned) hComp);
			return eRet;
		}
	}
	else
	{
		ALOGV("OMXCORE Warning: Free Handle called with no active instances\n");
	}
...
OMX_API OMX_ERRORTYPE OMX_APIENTRY OMX_SetupTunnel(OMX_IN OMX_HANDLETYPE outputComponent, OMX_IN OMX_U32 outputPort, OMX_IN OMX_HANDLETYPE inputComponent, OMX_IN OMX_U32 inputPort)
{
	...
	ALOGV("OMXCORE API: OMX_SetupTunnel Not implemented \n");
...
{
	/* Not supported right now */
	ALOGV("OMXCORE API: OMX_GetContentPipe Not implemented \n");
	return OMX_ErrorNotImplemented;
}
...
OMX_API OMX_ERRORTYPE OMX_GetRolesOfComponent(OMX_IN OMX_STRING compName, OMX_INOUT OMX_U32* numRoles, OMX_OUT OMX_U8** roles)
{
	...
	{
		ALOGV("ERROR: Both Roles and numRoles Invalid\n");
		eRet = OMX_ErrorBadParameter;
	}
