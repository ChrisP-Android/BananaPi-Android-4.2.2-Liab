      ...
      else
	{
	  ...
	  log_packet("DHCPACK", &mess->yiaddr, emac, emac_len, iface_name, hostname, mess->xid);  
	  {
        ...
        if(addr1) {
            ...
            #define WFD_P2P_DEVICE_ADDR  "wlan.wfdp2p.addr"
            android_printf(DEGUB_LEVLE_1, "client info: mac=%s,ip=%s", pmac, addr1 ? inet_ntoa(a) : "none");
            if(property_get(WFD_P2P_DEVICE_ADDR, device_addr, NULL)) {
                if(!strcmp(device_addr, pmac)|| !strncmp(device_addr, pmac, 6)) { // if match than set,rtk fully match,but brcm provpbc and dhcp mac is different. {//strcmp(device_addr, mac) if match than set
                    android_printf(DEGUB_LEVLE_1, "p2p device.");
                    property_set(WFD_DNSMASQ_PEER_IP, addr1 ? inet_ntoa(a) : "none");
                }
                else{
                	android_printf(DEGUB_LEVLE_1, "not p2p device.");
                }
