...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
    ...
    if(wpa_s->conf->p2p_go_swpsk) {
        p2p.p2p_go_swpsk_len = os_strlen(wpa_s->conf->p2p_go_swpsk);
		if (p2p.p2p_go_swpsk_len > sizeof(p2p.p2p_go_swpsk))
			p2p.p2p_go_swpsk_len = sizeof(p2p.p2p_go_swpsk);
		os_memcpy(p2p.p2p_go_swpsk, wpa_s->conf->p2p_go_swpsk,
			  p2p.p2p_go_swpsk_len);
    }
...
void wpas_p2p_update_config(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->conf->changed_parameters & CFG_CHANGED_P2P_GO_SWPSK) {
		p2p_set_go_swpsk(p2p, (u8 *) wpa_s->conf->p2p_go_swpsk,
				     wpa_s->conf->p2p_go_swpsk ?
				     os_strlen(wpa_s->conf->p2p_go_swpsk) :
				     0);
	}
