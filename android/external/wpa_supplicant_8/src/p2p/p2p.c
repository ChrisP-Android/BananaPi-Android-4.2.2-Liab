...
void p2p_build_ssid(struct p2p_data *p2p, u8 *ssid, size_t *ssid_len)
{
	...
	*ssid_len = P2P_WILDCARD_SSID_LEN + 2 + p2p->cfg->ssid_postfix_len;
	...
	os_memcpy(ssid, P2P_WILDCARD_SSID, P2P_WILDCARD_SSID_LEN);
	p2p_random((char *) &ssid[P2P_WILDCARD_SSID_LEN], 2);
	ssid[P2P_WILDCARD_SSID_LEN] = 'A';
	ssid[P2P_WILDCARD_SSID_LEN+1] = 'W';
	os_memcpy(&ssid[P2P_WILDCARD_SSID_LEN + 2],
		  p2p->cfg->ssid_postfix, p2p->cfg->ssid_postfix_len);
	*ssid_len = P2P_WILDCARD_SSID_LEN + 2 + p2p->cfg->ssid_postfix_len;
#endif	
...
{
	char *property = "12345678";
	p2p_build_ssid(p2p, params->ssid, &params->ssid_len);
	//p2p_random(params->passphrase, 8);	
	os_memcpy(params->passphrase, property, 8);
	wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: random generate Passphrase:[%s].", params->passphrase);
	if(p2p->cfg->p2p_go_swpsk)
	    os_memcpy(params->passphrase, p2p->cfg->p2p_go_swpsk, p2p->cfg->p2p_go_swpsk_len);
	    
	wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: user generate Passphrase from file:[%s]", params->passphrase);
	return 0;
}
...
void p2p_go_complete(struct p2p_data *p2p, struct p2p_device *peer)
{
	...
	if (go) {
		...
		//added by JM.		
		if(p2p->cfg->p2p_go_swpsk)
		    os_memcpy(res.passphrase, p2p->cfg->p2p_go_swpsk, p2p->cfg->p2p_go_swpsk_len);
...
//by sw platform
int p2p_set_go_swpsk(struct p2p_data *p2p, const u8 *psk, size_t len)
{
    if(len == 0 || psk == NULL) {
		wpa_printf(MSG_ERROR, "P2P: psk is invalid,SW GO PSK update failed!!!");
        return -1;
    }

    wpa_printf(MSG_INFO, "P2P: sw go psk[%s] update.", psk);
    if (len < sizeof(p2p->cfg->ssid_postfix)) {
        os_memcpy(p2p->cfg->p2p_go_swpsk, psk, len);
        p2p->cfg->p2p_go_swpsk_len = len;
    } else {
        wpa_printf(MSG_WARNING, "P2P: go psk len > buffer size !");
        os_memcpy(p2p->cfg->p2p_go_swpsk, psk, sizeof(p2p->cfg->p2p_go_swpsk));
        p2p->cfg->p2p_go_swpsk_len = sizeof(p2p->cfg->p2p_go_swpsk);
    }
	return 0;
}
