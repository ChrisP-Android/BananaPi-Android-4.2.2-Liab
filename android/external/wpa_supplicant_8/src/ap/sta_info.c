...
#define WFD_DEV_INFO_SIZE 9
	...
	if(sta->wfd_ie != NULL){
		wpa_printf(MSG_INFO, "sta->wfd_subelems is not NULL");
		...
		if(wfd_dev_info_hex[0] != 0)
			wpa_printf(MSG_INFO, "wfd_dev_info_hex =0x %s", wfd_dev_info_hex);
		else
			wpa_printf(MSG_INFO, "wfd_dev_info_hex is NULL");
		
	}
	else{
		wpa_printf(MSG_INFO, "sta->wfd_subelems is NULL");
	}		
#endif /* CONFIG_WIFI_DISPLAY */
	if (authorized) {
		if (dev_addr)
			wpa_msg(hapd->msg_ctx, MSG_INFO, AP_STA_CONNECTED
				MACSTR " p2p_dev_addr=" MACSTR "%s%s",
