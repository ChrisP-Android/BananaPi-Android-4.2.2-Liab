...
#define DEBUG(x...)   KLOG_ERROR("recovery", x)
...
Value* BurnBootFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (boot_num != UBOOT && boot_num != BOOT0) {
        return ErrorAbort(state, "%d is not a boot num, expects 0 or 1  (which respectively represents boot0 and uboot)", boot_num);
    }
    ...
    if (flash_type == FLASH_TYPE_UNKNOW){
        return ErrorAbort(state, "This platform does not support burning boot0\\uboot.\n");
    }

    Expr* pargv[2];
    Expr* bootexpr = malloc(sizeof(Expr));
    bootexpr->fn = Literal;
    bootexpr->name = bootbin;
    bootexpr->argc = 0;
    bootexpr->argv = NULL;
    //start and end are useless in PackageExtractFileFn;
    bootexpr->start = 0;
    bootexpr->end = 0;
    pargv[0] = bootexpr;

    Expr* devexpr = malloc(sizeof(Expr));
    devexpr->fn = Literal;
    devexpr->name = TMP_FEX_FILE_PATH;
    devexpr->argc = 0;
    devexpr->argv = NULL;
    //start and end are useless in PackageExtractFileFn;
    devexpr->start = 0;
    ...
    PackageExtractFileFn("PackageExtractFile",state,2,pargv);
