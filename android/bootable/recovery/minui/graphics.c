...
static int get_framebuffer(GGLSurface *fb)
{
   ...
   /* vi.red.offset     = 8;
      ...
      vi.transp.offset  = 0;
      vi.transp.length  = 8; */
      vi.red.offset = 16; /* in sun7i, the color sequence is ARGB (high -> low)*/
      vi.red.length = 8;
      vi.green.offset = 8;
      vi.green.length = 8;
      vi.blue.offset = 0;
    ...
    if (disp < 0) {
        perror("Error opening display driver");
    }

    ioctl(fd, FBIOGET_LAYER_HDL_0, &layer_hdl);
    ...
    r_width = layer_para.src_win.width;
    r_height = layer_para.src_win.height;
