    ...
    protected void attachTabToContentView(Tab tab) {
        ...
        if (parent != wrapper) {
            if (parent != null) {
                ...
                mainView.setNextFocusUpId(View.NO_ID);
            }
            wrapper.addView(mainView);
            //Author      : huanglong
            //Description : (BUGFIXED)when the focus leave the webview, set the first focus view to url input view.
            mainView.setNextFocusUpId(R.id.url);
        }
