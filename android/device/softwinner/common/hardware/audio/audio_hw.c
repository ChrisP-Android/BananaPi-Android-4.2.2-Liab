...
#define AUDIO_NAME_CODEC	"AUDIO_HDMI"
#define AUDIO_NAME_HDMI2	"AUDIO_HDMI2"
...
//			}
			...
			if (strstr(active_name_out, AUDIO_NAME_HDMI2))
			{
				adev->devices |= AUDIO_DEVICE_OUT_AUX_DIGITAL;
			}
		}
...
static int set_audio_devices_active_internal(struct sunxi_stream_out *stream, int in_out, int value)
{
	...
	switch(value & AUDIO_DEVICE_OUT_ALL) {
        ...
        case AUDIO_DEVICE_OUT_AUX_DIGITAL:
			// hdmi
			strcpy(devices, AUDIO_NAME_HDMI2);
            break;
	...
	if (in_out & AUDIO_OUT)
	{
		...
		if (strstr(prop_value_out, AUDIO_NAME_CODEC))
		{
			do_output_standby(out);
		}
		if (strstr(prop_value_out, AUDIO_NAME_HDMI2))
		{
			do_output_standby(out);
		}
	}
