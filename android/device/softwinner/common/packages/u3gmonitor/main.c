...
void handleUsbEvent(struct uevent *evt)
{
    ...
    if(!strcmp(evt->action, "add") && !strcmp(devtype, "usb_device")) {   
        ...
        */
		...
		asprintf(&cmd, "%s%s_%s &","/system/bin/usb_modeswitch.sh /system/etc/usb_modeswitch.d/",usb_vid,usb_pid);
		ALOGI("cmd=%s,", cmd);
        ret = system(cmd);
...
int main()
{
	...
	coldboot("/sys/devices/platform/sw_hcd_host0");
	coldboot("/sys/devices/platform/sw-ehci.1");
	coldboot("/sys/devices/platform/sw-ehci.2");
