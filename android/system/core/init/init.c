...
int get_sata_devname(char *name_buf, int len, char *cubie_c)
{
       int i, n;
       char namebuf[128];
       char cubiec[128];
       ...
       for (i = 'a'; i <= 'z'; i++ ){
               ...
               snprintf(cubiec, sizeof(cubiec), "/init.sd%c.rc", i);
	       memset(cubie_c, 0, len);
	       strncpy(cubie_c, cubiec, len);

	       ERROR("----------devname=%s----------", namebuf);
	       ERROR("----------cubiec=%s----------", cubie_c);
...
int main(int argc, char **argv)
{
    ...
    mkdir("/sys", 0755);
    mkdir("/cub", 0755);
    mkdir("/cub/block", 0755);
    mount("devtmpfs", "/cub/block", "devtmpfs", 0, NULL);
...
/*add by parker-----------------------------------begin*/
    ...
    if(get_sata_devname(devname, sizeof(devname), cubie_c)) {
		    ERROR("-----------找到sata节点-----");
	
	    if(check_cubie_header(devname)) {
		    init_parse_config_file(cubie_c);
		    ERROR("---------硬盘启动----------");
	    } else {
		    init_parse_config_file("/init.sun7i.rc");
		    ERROR("----------nand启动----------");
	    }
	
    } else {
	    ERROR("-----------没找到sata节点，nand启动--------------");
	    init_parse_config_file("/init.sun7i.rc");
    }
