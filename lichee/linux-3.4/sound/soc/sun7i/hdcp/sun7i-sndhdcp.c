...
static int sun7i_hdcp_set_audio_mode(struct snd_kcontrol *kcontrol,
...
static int sun7i_hdcp_get_audio_mode(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
...
static const struct snd_kcontrol_new sun7i_i2s_controls[] = {
	SOC_SINGLE_BOOL_EXT("I2s Or Pcm Audio Mode Select format", 0,
			sun7i_hdcp_get_audio_mode, sun7i_hdcp_set_audio_mode),
};

//set mclk and bclk division
static int sun7i_sndhdcp_hw_params(struct snd_pcm_substream *substream,
 ...
 * Card initialization
 */
static int sun7i_hdcp_init(struct snd_soc_pcm_runtime *rtd)
{
...
static struct snd_soc_ops sun7i_sndhdcp_ops = {
	.hw_params 		= sun7i_sndhdcp_hw_params,
};

static struct snd_soc_dai_link sun7i_sndhdcp_dai_link = {
	.name 			= "HDCP",
	.stream_name 	= "SUN7I-HDCP",
	.cpu_dai_name 	= "sun7i-hdcp.0",
	.codec_dai_name = "sndhdcp",
	.init 			= sun7i_hdcp_init,
	.platform_name 	= "sun7i-hdcp-pcm-audio.0",
	.codec_name 	= "sun7i-hdcp-codec.0",
	.ops 			= &sun7i_sndhdcp_ops,
};

static struct snd_soc_card snd_soc_sun7i_sndhdcp = {
	.name = "sndhdcp",
	.owner 		= THIS_MODULE,
	.dai_link 	= &sun7i_sndhdcp_dai_link,
	.num_links = 1,
};

static struct platform_device *sun7i_sndi2s_device;

static int __init sun7i_sndhdcp_init(void)
{
	int ret; 
    ...
    hdcp_used=val.val;
	type = script_get_item("i2s2_para", "i2s_master", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
        printk("[I2S] i2s_master type err!\n");
    }
	i2s_master = val.val;
	type = script_get_item("i2s2_para", "audio_format", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
        printk("[I2S] audio_format type err!\n");
    }
	audio_format = val.val;
	type = script_get_item("i2s2_para", "signal_inversion", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
    ...
    if (hdcp_used) {
		...
		if(!sun7i_sndi2s_device)
			return -ENOMEM;
		platform_set_drvdata(sun7i_sndi2s_device, &snd_soc_sun7i_sndhdcp);
...
static void __exit sun7i_sndhdcp_exit(void)
{
	if(hdcp_used) {
		hdcp_used = 0;
...
module_init(sun7i_sndhdcp_init);
module_exit(sun7i_sndhdcp_exit);

MODULE_AUTHOR("ALL WINNER");
MODULE_DESCRIPTION("SUN7I_sndhdcp ALSA SoC audio driver");
MODULE_LICENSE("GPL");
