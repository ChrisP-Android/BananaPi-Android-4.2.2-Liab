...
__asm__ __volatile__ ("lsrs r3, %0, #2\n\t"::"r"(size):"r3");
...
"broken_byte_loop:              \n\t"
	...
	"beq	add_broken_byte_one \n\t"  
	"cmp	ip, #2              \n\t"  //if ip/r12 = broken byte for checksum  equ two
    "beq    add_broken_byte_two \n\t " 
    "cmp	ip, #3              \n\t"
    "beq    add_broken_byte_tree\n\t " //if ip/r12 = broken byte for checksum  equ tree
    "b      sum_exit            \n\t"
...
"add_broken_byte_two:           \n\t"   //add two byte to sum
     ...
     "adds  r4, r4, r3          \n\t"
     "b     sum_exit            \n\t"
"add_broken_byte_tree:          \n\t"   //add tree byte to sum
     "ldr   r3, [r0 ,#0]        \n\t"
