 ...
 */
static NS16550_t serial_ports[8] = {
	...
	(NS16550_t)CONFIG_SYS_NS16550_COM7,
#else
	NULL,
#endif
#ifdef CONFIG_SYS_NS16550_COM8
	(NS16550_t)CONFIG_SYS_NS16550_COM8
#else
	NULL
#endif
};
...
int serial_init (void)
{
	...
	if(script_parser_fetch("uart_para", "uart_debug_port", &gd->uart_console, sizeof(int)))
		gd->uart_console = 0;
	if((gd->uart_console < 0) || (gd->uart_console > 7)){
		gd->uart_console = 0;
	}
	clock_divisor = calc_divisor(serial_ports[gd->uart_console]);
	NS16550_init(serial_ports[gd->uart_console], clock_divisor);
#endif
