...
__u32 NAND_GetBadBlockFlagPos(void)
{
	...
	return	NandStorageInfo.OptPhyOpPar.BadBlockFlagPosition;
...
__u32 NAND_GetVersion(__u8 *nand_version)
{
	...
	vsn = NAND_GetNandVersion();
	*(__u32 *)nand_version = vsn;
...
__u32 NAND_SetParam(void *buffer, uint length)
{
	return 0;
}


__s32 NAND_GetParam(void *buffer, uint length)
{
	__u32 i;
