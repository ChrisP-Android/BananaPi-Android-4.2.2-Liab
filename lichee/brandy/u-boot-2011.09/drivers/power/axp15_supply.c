...
static int axp15_set_dcdc1(int set_vol,int onoff)
{
    ...
    if(set_vol > 0)
    {
		...
		printf("the reg_value before is %x========\n",reg_value);
		reg_value &= ~(0x0f<<0);
		reg_value |= tem_reg_value;
		printf("the reg_value after1 is %x========\n",reg_value);
		...
		printf("the reg_value after2 is %x========\n",reg_value);
    }
	if(onoff<0)
	{
		return 0;
...
static int axp15_set_aldo1(int set_vol, int onoff)
{
	...
	if(set_vol > 0)
	{
		...
		{
			tem_reg_value = ((set_vol -2500)/100)+0x09;
		}
